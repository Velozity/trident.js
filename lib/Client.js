...
class TridentClient extends Client {
  ...
  async _registerEvents() {
    const directory = path.dirname(require.main.filename) + "\\events";
  ...
  async _registerCommands() {
    ...
    const directory = path.dirname(require.main.filename) + "\\commands";
    const customDirectoryFilter = [];
    ...
    for (const cmdFile of this.commandFiles) {
      ...
      if (this.commands.has(cmd.name) && cmd instanceof Command) {
        throw new Error(
